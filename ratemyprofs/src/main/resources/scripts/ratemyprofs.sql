...
-- -----------------------------------------------------
-- Table `ratemyprofs`.`USER`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `ratemyprofs`.`USER` (
  `ID_USER` INT NOT NULL AUTO_INCREMENT,
  `USERNAME` VARCHAR(45) NOT NULL,
  `PASSWORD` VARCHAR(45) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ratemyprofs`.`RATING` (
  ...
  INDEX `fk_RATING_USER2_idx` (`USER_ID_USER` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_RATING_USER2`
    ...
    REFERENCES `ratemyprofs`.`USER` (`ID_USER`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
